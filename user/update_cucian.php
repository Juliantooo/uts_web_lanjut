<?php 
    ...
    $sql = "UPDATE cucian SET berat='$berat',harga='$harga',keterangan_kain='$keterangan',jenis_paket='$jenis_paket',setrika='$setrika' WHERE id_user='$id_user' AND id_cucian='$id_cucian'";
    $query = mysqli_query($mysqli,$sql) or die ("Tidak ada database");

    if($query){
        echo "<script>alert('Edit data cucian sukses!')</script>";
        echo "<script>top.location='/uts_web_lanjut/user/tables.php'</script>";
    }else{
